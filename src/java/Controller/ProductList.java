...
public class ProductList extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("cateSelect")!=null&&!request.getParameter("cateSelect").contains("t")){
            ...
            if(idcateSelect!=0) P = Pdao.getProductByCate(idcateSelect);
            request.setAttribute("cs", idcateSelect);
        }
        // search
        if(request.getParameter("search")!=null){
            List<Product> searchL = new ArrayList<Product>();
            String strSearch = request.getParameter("search");
            searchL = Pdao.getProductByName(strSearch);
          if(!searchL.isEmpty()){
              P = searchL;  
          }
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("addToCart")!=null){
            if(session.getAttribute("Account")!=null){
                ...
                OrderDao.addOrder(pid, Pdao.getOnePro(pid));
                List<Order> carts = OrderDao.getAllOrder();
                session.setAttribute("listProduct", Pdao.getOnePro(pid));
                session.setAttribute("Cart", carts);
...
//                request.getRequestDispatcher("productList.jsp").forward(request, response);
        ...
        if(request.getParameter("btnDel")!=null){
            ...
            session.setAttribute("p", p);
            Account acc = (Account)session.getAttribute("Account");
            if(acc!=null&&acc.getRoleId()!=0) request.getRequestDispatcher("manager/productList.jsp").forward(request, response);
            else{
            request.getRequestDispatcher("productList.jsp").forward(request, response);}
        }
